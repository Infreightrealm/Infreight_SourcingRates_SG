...
- **Root Causes & Fixes (three-part):**
  1. **Price-owner radio not verified (upstream trigger).** The form filler clicked `label:has-text("I am the price owner")` with `force=True` and moved on. On Maersk's MDS web-component radios the real `<input type="radio">` lives in a shadow root, and a label force-click frequently fails to toggle it — Maersk then renders schedule-only results with no rates (every card `0.0 USD` / "not open"). Added `_verify_price_owner_selected()`: a shadow-root-piercing JS pass that locates the radio via the *smallest* element containing the wording (so ancestors like `<body>` or unrelated radios can't be grabbed), reports its real checked state, and — if the label click didn't register — clicks the actual input and re-verifies. Outcomes are logged (`checked` / `fixed` / `unchecked` / `unknown`), so the failure signature is now visible in logs instead of silent.
  2. **Sold-out/not-open cards consumed the 10-quote cap.** They were processed into price-less quotes that counted toward the "Already found 10 valid quotes" stop condition, so a page starting with not-open sailings could fill the quota before reaching a genuinely priced card — and those quotes were then dropped by the per-container filter anyway (`container_type=None`). Sold-out/not-open cards are now **skipped entirely** (same rule as ONE's "Notify Me" cards), freeing all cap slots for real quotes.
  3. **Fallback quotes were silently discarded.** When a priced card's per-container breakdown failed to render/parse, the fallback single-quote path left `container_type` unset (`None`), so the final `container_type == request.container_type` filter dropped it — a priced card produced zero results with no trace. The fallback now stamps the requested container type so the quote survives, and logs that it kept a single-type quote from the card total.
- **Diagnostic:** if a crawl ends with 0 priced quotes but skipped sold-out cards, a `[MAERSK] [DIAGNOSTIC]` log line now points at the price-owner verification result — making the "all cards rate-less" signature immediately identifiable in Railway logs.
- **Verified:** price-owner helper unit-tested across all 5 outcome paths, and its embedded JS validated at DOM level with jsdom — it selects the correct radio (ignoring a decoy radio and the "Someone else is the price owner" sibling), toggles only it, and re-reports checked. The live label-click→no-toggle behavior can only be fully confirmed against the real portal; the change degrades safely (verification failure logs and proceeds exactly as before).
- **Files:** `backend/carriers/maersk_connector.py`

### Hapag-Lloyd — Crawl Stalls by Typing a Port Locode Into the Login Email Field
- **Bug:** A Port Klang → Sydney search (and at least one other run) got stuck with the origin locode (`MYPKG`) typed into the Hapag Microsoft B2C login page's "E-mail Address" field, visible live in the VNC. The crawl never recovered on its own.
- **Root Cause:** Hapag can silently redirect an in-progress session back to `identity.hapag-lloyd.com` (the OAuth login portal) when the session token expires mid-crawl. `search_sailing_schedules` and `search_quotes` already check for this redirect immediately after navigation, but that check is a point-in-time snapshot — if the redirect fires a few seconds later (e.g. during modal dismissal or the ~10s "wait for Start Location field" loop), nothing re-checks before the loop times out and falls back to **"first input on the page."** On the login portal, the first input is the E-mail Address field, so the origin locode gets typed into it instead of a location box — a login field, not an autocomplete, so nothing can ever match and the crawl stalls.
- **Fix:** Added `_is_on_hapag_login_page()` (URL + login-field detection, reusing the same signals `login()` already uses) and `_ensure_not_stuck_on_login(target_url, page_label)`, which re-authenticates via `login()` when a redirect is detected, then (only if a distinct page like Schedule needs it — `login()` itself always finishes on the confirmed Quick Quote form) navigates back to `target_url`. Wired in as a safety-net check immediately before both blind "first input" fallbacks (Schedule page and Quote page), closing the time-of-check-to-time-of-use gap the existing post-navigation checks didn't cover. A failed recovery now raises a clear error instead of silently mistyping into an unrelated field.
- **Verified:** unit-tested all four paths — not-on-login-page (no unnecessary re-login), login-page-by-URL (recovers + re-navigates for Schedule), login-page-by-visible-field with failed re-login (returns False, no crash), and successful Quote-page recovery (correctly skips the redundant re-navigation).
- **Files:** `backend/carriers/hapag_lloyd_connector.py`

### Maersk — Regression: Other-Container-Type Sizes Went "Sold Out" in Multi-Container Searches
- **Bug:** In an all-container search (e.g. 40HQ + 40GP + 20GP), only ONE size (whichever one happened to succeed) returned real prices; the others came back "Sold out" in the Excel export. A **single**-container search for any of those same "sold out" sizes worked fine on its own.
- **Root Cause:** An earlier same-day commit (`565e660`, "scope cache key by container type") changed `run_full_search`'s cache key from `(origin, destination, departure_date)` to `(origin, destination, departure_date, container_type)`. Maersk's booking form adds **all three sizes to one submission** (confirmed in logs: 20 Dry Standard → +Add container → 40 Dry Standard → +Add container → 40 Dry High, all before Search is clicked), and a single crawl's price-breakdown cards already contain all three size columns — `_split_raw_quote_by_container_types` turns one crawl into quotes for every size. Scoping the cache by `container_type` broke that: each of the 3 job-service cycles (one per size) now missed the cache and triggered its **own independent full crawl** — tripling runtime and, critically, tripling exposure to Maersk's known flaky 0.0-price/"not open" card rendering (previously diagnosed, fix still pending). Each independent crawl is its own roll of the dice, so it became common for one crawl to succeed while the other two hit the flakiness and returned no usable quotes for their size — exactly "sold outing my other container types besides the one that always sits on the right."
- **Fix:** Reverted the cache key to the route (`origin`, `destination`, `departure_date`) only, restoring the single-crawl design: the first cycle's crawl caches the **full unfiltered split** (all sizes) under the route key; every cycle (including the first) then filters that shared result down to its own `request.container_type` before returning. A cache miss crawls once and serves all subsequent size-cycles from it, matching how the connector already behaves for a single-container search.
- **Verified:** simulated a 3-cycle multi-container search against the fixed cache contract — the underlying crawl fires exactly **once**, and each cycle correctly receives only its own container type's quotes (was 3 independent crawls under the regression).
- **Not addressed here:** the underlying Maersk 0.0-price/"not open" card-detection flakiness itself (diagnosed earlier, intentionally held off per prior instruction) is unchanged — it can still cause an occasional genuine miss on the single remaining crawl, just no longer 3x amplified by redundant re-crawling.
- **Files:** `backend/carriers/maersk_connector.py` (`run_full_search`)
