    ...
    async def search_quotes(self, request: RateSearchRequest) -> CarrierResultStatus:
        try:
            ...
            try:
                ...
                await self.page.keyboard.type(request.destination, delay=25)
                await self.page.wait_for_timeout(1000)
            ...
            try:
                ...
                try:
                    view_quote_btn = self.page.locator('text=/view Quote/i, button:has-text("View Quote")').first
                    await view_quote_btn.wait_for(state="visible", timeout=3000)
                    await view_quote_btn.click(force=True)
                    print("[ONE] Clicked 'view Quote' directly from calendar")
                except Exception:
                    print("[ONE] 'view Quote' not found, looking for an available date...")
                    try:
                        price_locator = self.page.locator('text=/\\d+(\\.\\d+)?[K|M]/i, div[role="option"][aria-label^="Choose"]').first
                        await price_locator.wait_for(state="visible", timeout=15000)
                        await price_locator.click(force=True)
                        print("[ONE] Clicked an available date on the calendar")
                    except Exception as inner_e:
                        print(f"[ONE] Calendar click failed, ignoring: {inner_e}")
            ...
            if not await self._click_submit():
                print("[ONE] _click_submit returned False, but proceeding in case form was already submitted.")
