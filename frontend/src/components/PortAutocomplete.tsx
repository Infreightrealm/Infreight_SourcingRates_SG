  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {showDropdown && suggestions.length > 0 && (
        <div className="absolute z-50 left-0 right-0 mt-2 bg-[#1a1c2e] border border-white/10 rounded-xl shadow-2xl overflow-hidden backdrop-blur-xl max-h-64 overflow-y-auto">
          {suggestions.map((port) => (
            <button
              key={port.code}
              type="button"
              onClick={() => handleSelect(port)}
              className="w-full px-4 py-3 text-left hover:bg-white/5 border-b border-white/5 last:border-0 transition-colors flex flex-col gap-0.5"
            >
              <div className="flex items-center justify-between">
                <span className="text-white font-medium text-sm">{port.name}</span>
                <span className="text-blue-400 font-mono text-[10px] bg-blue-500/10 px-1.5 py-0.5 rounded uppercase">{port.code}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-white/40 text-xs">{port.country}</span>
